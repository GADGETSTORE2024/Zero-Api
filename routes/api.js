...
var error_bot = __path + '/view/error.html' // Error
...
router.get('/api/dowloader/gdrive', cekKey, async (req, res, next) => {
	var url = req.query.url
...
router.get('/api/ai/gbard', premKey, async (req, res, next) => {
    ...
    const bot = await axios.get(`https://api.ssateam.my.id/api/gemini?message=${encodeURIComponent(text)}`);
    const data = bot.data.data.response;
    //console.log(data);
    res.json({
        status: true,
        creator: `${creator}`,
...
router.get('/api/maker/circle', cekKey, async (req, res) => {
    ...
    if (!img) return res.json({
        status: false, creator: 'Zero-Api', message: "[!] cek kembali url image"
    })
...
router.get('/api/maker/darkness', cekKey, async (req, res) => {
    ...
    if (!n) return res.json({
        status: false, creator: 'Zero-Api', message: "[!] parameter no nombor sahaja"
    })
...
router.get('/api/maker/resize', cekKey, async (req, res) => {
    ...
    if (!wid) return res.json({
        status: false, creator: 'Zero-Api', message: "[!] parameter width nombor sahaja"
    })
    if (!hei) return res.json({
        status: false, creator: 'Zero-Api', message: "[!] parameter height nombor sahaja"
    })
...
router.get('/api/maker/welcome1', cekKey, async (req, res, next) => {
    ...
    if (!name) return res.json({
        status: false, creator: 'Zero-Api', message: "[!] masukan parameter name"
    })
    if (!grup) return res.json({
        status: false, creator: 'Zero-Api', message: "[!] masukan parameter gpname"
    })
    if (!member) return res.json({
        status: false, creator: 'Zero-Api', message: "[!] masukan parameter member"
    })
    if (!pp) return res.json({
        status: false, creator: 'Zero-Api', message: "[!] masukan parameter pp"
    })
    if (!bg) return res.json({
        status: false, creator: 'Zero-Api', message: "[!] masukan parameter bg"
    })

    if (!imgpp) return res.json({
        status: false, creator: 'Zero-Api', message: "[!] cek kembali url image pp"
    })
    if (!bgimg) return res.json({
        status: false, creator: 'Zero-Api', message: "[!] cek kembali url image bg"
    })
...
router.get('/api/maker/goodbye1', cekKey, async (req, res, next) => {
    ...
    if (!imgpp) return res.json({
        status: false, creator: 'Zero-Api', message: "[!] masukan parameter pp Link pp dengan betul"
    })
    if (!bgimg) return res.json({
        status: false, creator: 'Zero-Api', message: "[!] masukan parameter pp Link bg dengan betul"
    })
...
router.get('/api/linkshort/tinyurl', cekKey, async (req, res, next) => {
    ...
    TinyURL.shorten(link, function(link, err) {
        if (err) return res.sendFile(error_bot)
        res.json(loghandler.error)
        limitapikey(req.query.apikey)
        res.json({
            status: true,
...
router.get('/api/info/waktuksolatmy', cekKey, async (req, res, next) => {
    alip.watuksolatmy()
    .then(data => {
        if (!data.Tarikh) return res.sendFile(error_bot)
        res.json(loghandler.error)
        limitapikey(req.query.apikey)
        res.json({
            status: true,
